...
function handleUserLeft(msg) {
    $('#usersList').find('span#user-'+msg.userName).remove();
    var msgForChat = {
            "source": 'SYSTEM',
...
function setUsersAmount(amount){
    $('#usersList span.usersAmount a').text(amount);
}
...
$(function() {
    ...
    socket.on('welcome', function(msg) {
        $("h5.nickName strong").text("Hello " + msg.userName);
        $('form').hide();
